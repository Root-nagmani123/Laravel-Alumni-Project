  ...
  onView: function (storyId) {
    ...
    setTimeout(() => {
      ...
      deleteBtn.onclick = function () {
        const activeItem = document.querySelector(".story-viewer-item.active");
        if (!activeItem) return;

        const storyIdAttr = activeItem.getAttribute("data-id") || '';
      ...
      document.querySelector(".zuck-modal").addEventListener("click", () => {
        const btn = document.querySelector("#deleteStoryBtn");
        if (btn) btn.remove();
      });
