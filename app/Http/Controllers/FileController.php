...
class FileController extends Controller
{
    ...
    public function secureShow($type, $path)
    {
        ...
        // Group and forum should be viewable by both admin and users
        if (in_array($type, ['group', 'forum']) && !Auth::guard('admin')->check() && !Auth::guard('user')->check()) {
            abort(403, 'Unauthorized for ' . $type);
        }
        // Posts and stories are for users only
        if (in_array($type, ['post', 'story']) && !Auth::guard('user')->check()) {
            abort(403, 'Unauthorized for ' . $type);
        }
        ...
        if (!file_exists($fullPath)) {
            ...
            if (!file_exists($fullPath)) {
                // Handle old forum/group images that might be stored as basename only
                // Check if this is a forum or group type and the path doesn't already include the folder
                if ($type === 'forum' && !str_contains($path, 'uploads/images/forums_img/')) {
                    $folderPath = 'uploads/images/forums_img/';
                ...
                } elseif ($type === 'group' && !str_contains($path, 'uploads/images/grp_img/')) {
                    $folderPath = 'uploads/images/grp_img/';
                    $fullPath = storage_path('app/private/' . $folderPath . $path);
                    if (!file_exists($fullPath)) {
                        $fullPath = storage_path('app/public/' . $folderPath . $path);
                        if (!file_exists($fullPath)) {
