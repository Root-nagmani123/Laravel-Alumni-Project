<div>
    ...
    <!-- Offcanvas header -->
    <div class="offcanvas-header d-flex justify-content-between align-items-center bg-white shadow-sm rounded-top px-3 py-2">
        <h5 class="offcanvas-title fw-bold mb-0">Mentor/Mentee Conversation</h5>
        <button type="button" class="btn btn-light btn-circle d-flex align-items-center justify-content-center border-0 shadow-sm" style="width:36px; height:36px;" data-bs-dismiss="offcanvas" aria-label="Close">
            ...
            <div class="os-viewport os-viewport-native-scrollbars-invisible" style="overflow-y: scroll;">
                <div class="os-content" style="padding: 0px 24px 16px; height: 100%; width: 100%;">
                    ...
                    <div class="position-relative mb-3">
                        <input type="text" class="form-control rounded-pill ps-5 pe-3 py-2 shadow-sm" placeholder="Search users..." wire:model.live="search" aria-label="Search users" />
                        <span class="position-absolute top-50 start-0 translate-middle-y ms-3 text-muted" style="z-index:2;">
                    ...
                    <ul class="list-unstyled mb-0">
                        ...
                        <li class="d-flex align-items-center gap-3 px-2 py-2 mb-1 rounded position-relative select_user_click toast-btn user-id{{ $chat->member_id }} {{ $selectedChat == $chat->member_id ? 'bg-danger text-white shadow-sm' : 'bg-white' }} chat-list-item"
                            ...
                            <div class="position-relative" style="min-width:48px;">
                                <img class="avatar-img rounded-circle border border-2 {{ $isOnline ? 'border-success' : 'border-secondary' }}" src="{{ $profileImage }}" alt="{{ $chat->name }}" style="width:44px; height:44px; object-fit:cover;">
                                @if($isOnline)
                                    <span class="position-absolute bottom-0 end-0 bg-success border border-white rounded-circle" style="width:13px; height:13px;" title="Online"></span>
                                @else
                                    <span class="position-absolute bottom-0 end-0 small text-dark bg-light border border-secondary rounded-circle px-1" style="font-size:10px;" title="Last seen">{{ $lastSeen ?: 'Offline' }}</span>
                                @endif
                            </div>
                            <div class="flex-grow-1 overflow-hidden">
                                <div class="d-flex align-items-center gap-2">
                                    <span class="fw-semibold text-truncate {{ $selectedChat == $chat->member_id ? 'text-white' : 'text-dark' }}" style="max-width:120px;">{{ $chat->name }}</span>
                                    @if ($unreadCount > 0 && $selectedChat != $chat->member_id)
                                        <span class="badge bg-danger ms-1" id="unread-count-{{ $chat->member_id }}">{{ $unreadCount }}</span>
                                    @endif
                                </div>
                                <div class="small text-truncate {{ $selectedChat == $chat->member_id ? 'text-light' : 'text-secondary' }}">{{ $chat->role_type ?? '' }}</div>
                            </div>
                        </li>
                        @endforeach
                    </ul>
        ...
        <div class="toast-container toast-chat d-flex gap-3 align-items-end">
            ...
            <div id="chatToast-{{ $selectedChat }}" class="toast mb-0 bg-mode fade show" role="alert"
                ...
                <div class="toast-body collapse show" id="collapseChat-{{ $selectedChat }}">
                   <div class="chat-conversation-content custom-scrollbar px-3 py-3" style="overflow-y:auto; max-height:320px; background:#efeae2; border-radius:12px;" id="chat-container-{{ $selectedChat }}">
                        ...
                        @foreach ($groupedMessages as $date => $dailyMessages)
                            ...
                            <div class="text-center position-relative my-3">
                                ...
                                <hr class="position-absolute top-50 start-0 w-100 border-0 border-top border-light opacity-25">
                            </div>
                                ...
                                @if ($msg->sender_id != auth()->guard('user')->id())
                                    ...
                                    <div class="d-flex align-items-end mb-2">
                                        <div class="flex-shrink-0 avatar avatar-xs me-2">
                                            <img class="avatar-img rounded-circle" src="{{ $profileImage }}" width="36" height="36" alt="">
                                        </div>
                                        <div class="d-flex flex-column align-items-start">
                                            <div class="bg-light text-secondary p-2 px-3 rounded-2 shadow-sm" style="max-width:320px;">
                                                @if($msg->file_path)
                                                    ...
                                                    @else
                                                        <a href="{{ asset('storage/'.$msg->file_path) }}" target="_blank" class="d-block text-decoration-none">
                                ...
                                @else
                                    ...
                                    <div class="d-flex justify-content-end align-items-end mb-2">
                                        <div class="d-flex flex-column align-items-end">
                                            <div class="p-2 px-3 rounded-2 shadow-sm" style="background-color:#dcf8c6; max-width:250px; border-bottom-right-radius:4px;">
                                                @if($msg->file_path)
                                                    @if($msg->file_type == 'image')
                                                        <img src="{{ asset('storage/'.$msg->file_path) }}" class="rounded mb-1" style="max-width:200px;">
                                                    @elseif($msg->file_type == 'video')
                                                    ...
                                                    @else
                                                        <a href="{{ asset('storage/'.$msg->file_path) }}" target="_blank" class="text-dark d-block text-decoration-none">
                                                            <i class="fa-solid fa-file me-1"></i> {{ basename($msg->file_path) }}
                                                        </a>
                                                    @endif
                                                @endif
            ...
            @if(str_starts_with($attachment->getMimeType(), 'image/'))
                <div class="position-relative d-inline-block">
                    ...
                    <button type="button"
                            ...
                            class="btn btn-sm btn-light border rounded-circle position-absolute top-0 end-0 m-1 shadow"
            ...
            @else
                ...
                <div class="d-flex align-items-center p-2 px-3 bg-white rounded-3 shadow-sm border">
                    ...
                    <div class="text-truncate" style="max-width:160px;">
                        ...
                        <div class="text-muted small">{{ number_format($attachment->getSize()/1024,1) }} KB</div>
                    </div>
                    <button type="button"
                            wire:click="$set('attachment', null)"
...
@endif
                    ...
                    <div class="chat-input mt-3 position-relative">
                        <form wire:submit.prevent="submit" class="d-flex align-items-center position-relative gap-2">
                            <div class="input-wrapper flex-grow-1 d-flex align-items-center bg-white rounded-pill px-3 py-2 shadow-sm border border-1">
                                <input type="file" id="chat-attachment" class="d-none" wire:model="attachment" accept="image/*,.pdf,.doc,.docx,.xls,.xlsx,.zip,.mp4,.mov,.avi,.mkv" />
                                <button type="button" onclick="document.getElementById('chat-attachment').click()" class="btn btn-light btn-sm me-2 d-flex align-items-center justify-content-center" style="width:36px; height:36px; border-radius:50%; font-size:16px; color:#555;" aria-label="Attach file">
                                    <i class="fa-solid fa-paperclip"></i>
                                </button>
                                <textarea class="form-control border-0 flex-grow-1 bg-transparent" rows="1" wire:model.defer="newMessage" wire:key="chat-message-input-{{ now() }}" placeholder="Type a message..." style="resize:none; overflow:hidden; outline:none; font-size:14px; line-height:1.4;"></textarea>
                            </div>
                            <button type="submit" class="btn send-btn d-flex align-items-center justify-content-center shadow-sm" style="width:44px; height:44px; border-radius:50%; background:#af2910; color:#fff; border:none;" aria-label="Send message">
