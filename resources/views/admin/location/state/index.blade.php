...
@section('title', 'States - Alumni | Lal Bahadur Shastri National Academy of Administration')
        ...
        <div class="row align-items-center">
            <div class="col-12">
                <div class="d-sm-flex align-items-center justify-space-between">
                    <h4 class="mb-4 mb-sm-0 card-title">States</h4>
                    ...
                    <div class="row">
                        ...
                        <div class="col-6">
                            <div class="float-end gap-2">
                                <a href="{{ route('admin.location.state.create') }}" class="btn btn-primary">+ Add State</a>
                            </div>
                        </div>
                    </div>
                    <hr>
                            ...
                            <tbody>
                                ...
                                <tr class="odd">
                                    ...
                                    <td>
                                        ...
                                        <form action="{{ route('admin.location.state.destroy', $state->id) }}" method="POST" style="display:inline;">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="btn btn-danger text-white btn-sm delete-btn"
                                                    onclick="return confirm('Are you sure you want to delete?')"
                                                    {{ $state->status == 1 ? 'disabled' : '' }}
                                    ...
                                    <td>
                                        <div class="form-check form-switch d-inline-block">
                                            <input class="form-check-input status-toggle" type="checkbox" role="switch"
                                                data-table="states" data-column="active_inactive" data-id="{{ $state->id }}"
...
<script>
    $(document).ready(function () {
        ...
        $('.status-toggle').change(function (e) {
            ...
            $.ajax({
                url: "{{ route('admin.location.state.toggle-status', ':id') }}".replace(':id', stateId),
                type: 'POST',
                data: {
                    _token: '{{ csrf_token() }}'
                },
                success: function (response) {
                    ...
                    const deleteBtn = $(`.delete-btn[data-state-id="${stateId}"]`);
                    if (currentState) {
